...
function Navbar() {
    ...
    const name = isAuthenticated?localStorage.getItem("name").split(' ')[0]:"";
    ...
    return (
        <div className='w-full red navbar_main flex justify-between' style={header==="header"?{backgroundColor:"transparent", borderBottom:"1px solid white"}: {backgroundColor:"#252930"}}>
            ...
            <div className="gallary_name nav_laptopview">
                {isloggedin?<span className='nav_span mx-0.5'> Welcome, {name}</span>:<></>} 
                <span className='nav_span mx-0.5' style={{cursor:"pointer"}} onClick={()=>{isloggedin?navigate("/createpost"):navigate("/login")}}> Post</span>
            ...
            <div className="nav_mobileview" >
                ...
                <div className='nav_dropdown' style={header==="header"?{backgroundColor:"transparent", border:"1px solid white"}: {backgroundColor:"#252930"}}>
                    {isloggedin?<span className='nav_span_1' style={{borderBottom:"1px solid white"}}> Welcome, {name}</span>:<></>}
                    <span className='nav_span_1' style={{cursor:"pointer", borderBottom:"1px solid white"}} onClick={()=>{isloggedin?navigate("/createpost"):navigate("/login")}}> Post</span>
